...
Applikationen drivs idag på **gratiskonton** registrerade på extern utvecklare:
...
- Vercel Hobby tillåter inte kommersiellt bruk enligt deras användarvillkor.
- Ingen SLA, ingen backup-policy, inga loggar äldre än 1 dag.
- SSO mot Curagos M365 är inte möjligt på gratisnivåerna.

**Användarstatus per maj 2026:**
- 10 av Curagos ca 30 konsulter har skapat konto.
- 6 har påbörjat utbildningen, 2 är klara med alla sex moduler.
- Genomsnittlig quiz-poäng: 98,8 av 100 (17 quiz genomförda).
- Aktiviteten är låg de senaste 30 dagarna (2 aktiva användare).

Plattformen används alltså på riktigt, men i blygsam skala. Det är tillräckligt för att motivera en ordnad förvaltning, och liten nog att flytten kan göras utan driftstörning.
...
```
Curago Org (ägare)
├── GitHub Org: [IT-leverantör] eller Curago AB
...
| **Domän** | `.se` | **0 kr** | Subdomän till `curago.se` |
...
> Dimensioneringen ovan är tilltagen för Curagos hela konsultkår (~30 personer) plus framtida tillväxt. Supabase Pro har god marginal till databas- och bandbreddstaket även vid full användning.

**Förvaltningsarvode (förslag):**
- Löpande underhåll, säkerhetspatchar, dependency-uppdateringar, mindre innehållsändringar: **fast månadsvolym 2–4 h/mån**.
- Större ändringar (nya moduler, designändringar, nya features): **pris per timma** enligt avtal.
...
4. **Domän**: Registrera `utbildning.curago.se` som CNAME (eller välj annan subdomän).
...
## 5. Inloggning med Single Sign-On (SSO) mot Microsoft 365 (Entra ID)

Idag används **e-post + lösenord** via Supabase Auth, begränsat till `@curago.se`. För att konsulter ska kunna logga in med sitt vanliga Microsoft-konto (samma SSO som Outlook/Teams) sätter Curagos IT-leverantör upp **SAML 2.0 mellan Supabase och Microsoft Entra ID**.

> **Förutsättning:** Supabase SSO kräver **Pro plan + SSO add-on** ($75/mån extra) eller att man använder Supabases inbyggda **Azure OAuth-provider** som ingår i Pro-planen utan extratillägg. För Curagos storlek räcker **Azure OAuth-providern** då den ger samma användarupplevelse (en knapp "Logga in med Microsoft") utan SAML-priset.
...
4. Redirect URL: `https://<projekt-id>.supabase.co/auth/v1/callback` (Web)
5. Klicka **Register**
...
För Curagos storlek (~30 konsulter) räcker Variant A.

### 5.3 Domänlås
Oavsett variant är förslaget att behålla kodregeln att endast `@curago.se`-adresser släpps in. Det är en extra säkerhet ifall någon felkonfigurerar Azure-appen som multi-tenant.
...
| **Hemligheter** | `.env` i Vercel, aldrig i repot. Rotera Supabase service-role key vid personalbyte eller vid byte av IT-leverantör |
...
| **2FA** | Tvinga 2-faktorsautentisering på alla GitHub/Vercel/Supabase-konton |
...
| Större ändringar (nya moduler, features) | Beställning | Pris per timma |
| Backup-verifiering | Kvartal | 0,5 h |
| Innehållsöversyn baserat på användardata (vilka moduler folk fastnar på, quizfrågor som svarats fel på) | Halvår | 2 h |
...
2. Önskad subdomän (förslag: `utbildning.curago.se` eller `ai-utbildning.curago.se`)?
3. SSO-variant: **A (Azure OAuth, ingår)** eller B (SAML, +$75/mån)?
4. Förvaltningsavtal: fast månadsvolym eller löpande timdebitering?

**Löpande kostnad för Curago:**
- **Drift**: ca **500–550 kr/mån** (Supabase Pro + Vercel Pro + subdomän).
- **Förvaltning**: enligt avtal, riktvärde **2–4 h/mån** som fast månadsvolym.
- **Total**: ca **15 000–25 000 kr/år**.
